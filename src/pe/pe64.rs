...
impl OPTIONAL_HEADER_STANDARD_FILE {
    fn optional_header_standard_file(binary: Vec<u8>, DOS_COFF: usize) -> OPTIONAL_HEADER_STANDARD_FILE {
        let ohdr_sf: Ohdr_SF = Ohdr_SF::ohdr_sf_struct(binary[DOS_COFF..DOS_COFF+0x18].to_vec());
...
impl OPTIONAL_HEADER_WINDOWS_SPECIFIC_FIELDS {
    fn optional_header_windows_specific_fields(binary: Vec<u8>, DOS_COFF_OSF: usize) -> OPTIONAL_HEADER_WINDOWS_SPECIFIC_FIELDS {
        let ohdr_wsf = Ohdr_WSF::ohdr_wsf_struct(binary[DOS_COFF_OSF..DOS_COFF_OSF+0x58].to_vec());
...
// Offset 0x58
struct Ohdr_WSF {
    ImageBase: Xword,  // The preferred address of the first byte of image when loaded into memory. The default for DLLs is 0x10000000. The default for Windows is 0x00400000.
    SectionAlignment: Dword,  // The alignment (in bytes) of sections when they are loaded into memory.
    FileAlignment: Dword,  // The alignment factor (in bytes) that is used to align the raw data of sections in the image file.
    ...
    SizeOfStackReserve: Xword,  // The size of the stack to reserve.
    SizeOfStackCommit: Xword,  // The size of the stack to commit
    SizeOfHeapReserve: Xword,  // The size of the local heap space to reserve.
    SizeOfHeapCommit: Xword,  // The size of the local heap space to commit.
...
impl Ohdr_WSF {
    ...
    fn print(&self) {
        ...
        eprintln!("MinorSubsystemVersion -> {:02x}", self.MinorSubsystemVersion);
        ...
        eprintln!("DllCharacteristics -> 0x{:02x}", self.DllCharacteristics);
        ...
        eprintln!("LoaderFlags -> {:032b}", self.LoaderFlags);
...
struct Ohdr_DD {
    ...
    ImportTable: Xword,
    ResourceTable: Xword,
    ExceptionTable: Xword,
    CertificateTable: Xword,
    BaseRelocationTable: Xword,
    Debug: Xword,
    Architecture: Xword,
    GlobalPtr: Xword,
    TLSTable: Xword,
    LoadConfigTable: Xword,
    BoundImport: Xword,
    IAT: Xword,
    DelayImportDescriptor: Xword,
    Null: Xword,
}
impl Ohdr_DD {
...
pub fn optional_header_pe32_plus_detail(buf: Vec<u8>, i: usize) {
